...
record RequestProcessor(Connection connection, WebServer webServer) implements Runnable {
    ...
    public void run() {
        try {
            ...
            switch (parsed) {
                case WebResponse errorResponse -> webServer.sendResponseAndClose(connection, null, errorResponse);
                case WebRequest request -> {
                    try {
                        if (request.getMethod().equals(HttpMethod.GET) &&
                            ...
                            if (webServer.websocketManager.pathExists(request.getPath())) {
                                ...
                                webServer.sendResponse(connection.getChannel(), request, HttpResponseCode.SWITCHING_PROTOCOLS, new ByteArrayInputStream(new byte[0]), new HttpHeader("Upgrade", "websocket"), new HttpHeader("Connection", "Upgrade"), new HttpHeader("Sec-WebSocket-Accept", accept));
                                ...
                                webServer.sendResponse(connection.getChannel(), request, HttpResponseCode.NOT_FOUND, new ByteArrayInputStream(new byte[0]));
                            }
                    ...
                    if ((request.getMethod().equals(HttpMethod.GET) || request.getMethod().equals(HttpMethod.HEAD)) && request.getPath().startsWith("/static/")) {
                        ...
                        if (file.getCanonicalPath().startsWith(webServer.getStaticDir().getCanonicalPath())) {
                            try {
                                webServer.sendResponseAndClose(connection, request, HttpResponseCode.OK, new FileInputStream(file), new HttpHeader("Content-Type", MimeType.getByFilename(file.getCanonicalPath()).getMimetype()));
                            } catch (FileNotFoundException ignored) {
                                webServer.sendResponseAndClose(connection, request, webServer.errorHandlerManager.handle(HttpResponseCode.NOT_FOUND, request.getPath(), request.getArgs(), request.getHeaders()));
                            } catch (Throwable ignored) {
                                webServer.sendResponseAndClose(connection, request, webServer.errorHandlerManager.handle(HttpResponseCode.INTERNAL_SERVER_ERROR, request.getPath(), request.getArgs(), request.getHeaders()));
                            }
                        } else {
                            webServer.sendResponseAndClose(connection, request, webServer.errorHandlerManager.handle(HttpResponseCode.BAD_REQUEST, request.getPath(), request.getArgs(), request.getHeaders()));
                        }
                        return;
                    } else if ((request.getMethod().equals(HttpMethod.GET) || request.getMethod().equals(HttpMethod.HEAD)) && request.getPath().equals("/favicon.ico")) {
                        lookupSpecialFiles(request, new File(webServer.getStaticDir().getParent(), "favicon.ico"));
                        return;
                    } else if ((request.getMethod().equals(HttpMethod.GET) || request.getMethod().equals(HttpMethod.HEAD)) && request.getPath().equals("/robots.txt")) {
                        lookupSpecialFiles(request, new File(webServer.getStaticDir().getParent(), "robots.txt"));
                        return;
                    }
                    if (!request.getMethod().equals(HttpMethod.OPTIONS)) {
                        ...
                        if (handlerBundle != null) {
                            ...
                            if (webServer.handlerManager.hasPath(request.getPath())) {
                                webServer.sendResponseAndClose(connection, request, webServer.errorHandlerManager.handle(HttpResponseCode.METHOD_NOT_ALLOWED, request.getPath(), request.getArgs(), request.getHeaders()));
                            } else {
                                webServer.sendResponseAndClose(connection, request, webServer.errorHandlerManager.handle(HttpResponseCode.NOT_FOUND, request.getPath(), request.getArgs(), request.getHeaders()));
                            }
                            return;
                        }
                        webServer.sendResponseAndClose(connection, request, convertToResponse(request, response));
                    } else {
                        if (webServer.handlerManager.getHandler(HttpMethod.OPTIONS, request.getPath()) != null) {
                            webServer.sendResponseAndClose(connection, request, convertToResponse(request, webServer.handlerManager.getHandler(HttpMethod.OPTIONS, request.getPath()).invoke(request)));
                        } else if (request.getPath().equals("*")) {
                            List<String> list = new ArrayList<>();
                            for (HttpMethod i : webServer.handlerManager.getUsedMethods()) {
                                list.add(i.name());
                            }
                            webServer.sendResponseAndClose(connection, request, WebResponse.create().setResponseCode(HttpResponseCode.NO_CONTENT).addHeader(new HttpHeader("Allow", String.join(", ", list))).build());
                        } else {
    ...
    private void lookupSpecialFiles(WebRequest request, File file) throws FileNotFoundException {
        if (file.exists()) {
            webServer.sendResponseAndClose(connection, request, WebResponse.create().setContent(new FileInputStream(file)).addHeader(new HttpHeader("Content-Type", MimeType.getByFilename(file.getName()).getMimetype())).build());
        } else {
            webServer.sendResponseAndClose(connection, request, webServer.errorHandlerManager.handle(HttpResponseCode.NOT_FOUND, request.getPath(), request.getArgs(), request.getHeaders()));
        }
    }
